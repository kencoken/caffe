classdef ConvNetEncoder < handle & featpipem.directencode.GenericDirectEncoder
    ...
    methods
        function obj = ConvNetEncoder(varargin)
            ...
            p.addParamValue('output_blob_names', {'fc7'}, @iscell);
            ...
            % set caffe to by default use the cpu and be in test mode
            caffe('set_mode_cpu', obj.net_handle_);
            caffe('set_phase_test', obj.net_handle_);
        end
        ...
        function dim = get_output_dim(obj, run_mode)
            ...
            dim_mul = obj.augmentation_helper_.get_output_dim_mul(run_mode);
            dim = cellfun(@(x) x*dim_mul, dim, 'UniformOutput', false);
        ...
        function weights = get_weights(obj)
            weights = caffe('get_weights', obj.net_handle_);
        end
        ...
        function set.param_file(obj, value)
            ...
            if obj.net_handle_ > 0, obj.initNet_(); end %#ok<MCSUP>
        end
        ...
        %--
        function set.backend(obj, value) %#ok<INUSD>
            error('backend is readonly');
        end
        function value = get.backend(obj) %#ok<MANU>
    ...
    methods(Access=protected)
        ...
        function dim = get_net_output_dim_(obj)
            ...
            dim = caffe('get_blob_size', obj.output_blob_names, obj.net_handle_);
            dim = cellfun(@(x) prod(x(2:end)), dim, 'UniformOutput', false);
        end
    end
    ...
    methods(Static=true)
        function obj = loadobj(a)
            ...
            obj.output_blob_names = a.output_blob_names;
