...
static void init(MEX_ARGS) {
  if ((nrhs < 2) || (nrhs > 4)) {
    ostringstream error_msg;
    error_msg << "Wrong number of arguments. Usage: caffe('init', model_def_file, model_file"
  ...
  if (nrhs >= 3) {
    input_count = mxGetScalar(prhs[2]);
  }
  if (nrhs >= 4) {
    net_handle = static_cast<int32_t>(mxGetScalar(prhs[3]));
  }
  ...
  if (net_handle == GLOBAL_NET) {
    net_.reset(new Net<float>(string(param_file), input_count));
    ...
    shared_ptr<Net<float > > net(new Net<float>(string(param_file), input_count));
